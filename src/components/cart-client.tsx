...
export function CartClient() {
  const { cart, total, remaining, spentRatio, totalQuantity, updateQuantity, removeItem, hydrated, currency, budgetSourceLabel, startingBudgetSek } = useGame();
      ...
      <section className="bg-[var(--paper)] py-20 text-[var(--ink)]">
        <div className="shell grid min-h-[58vh] place-items-center">
          <div className="max-w-xl text-center">
            ...
            <h1 className="mt-7 font-display text-6xl">Din förmögenhet är orörd</h1>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1.35fr_.65fr] lg:items-start">
                  ...
                  <div className="flex flex-col justify-between gap-5 p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-2 text-sm text-black/50">{formatMoneyFromSek(product.priceSek, currency)} per styck</p>
                      </div>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-4">
                      <QuantityControl
                        ...
                        onIncrease={() => {
                          ...
                          if (!result.ok) showToast(`Du saknar ${formatMoneyFromSek(result.missing, currency)} för ett exemplar till.`, "error");
                        }}
                        label={`Antal ${product.name}`}
                      />
                      <div className="text-right">
                        <p className="text-xs text-black/45">Delsumma</p>
                        <p className="font-display text-3xl">{formatMoneyFromSek(product.priceSek * item.quantity, currency)}</p>
                      </div>
                    </div>
                  </div>
                </article>
          ...
          <aside className="sticky top-24 space-y-5">
            <div className="rounded-2xl bg-[var(--ink)] p-6 text-white shadow-xl">
              <p className="text-xs uppercase tracking-[0.18em] text-white/45">{budgetSourceLabel}</p>
              <dl className="mt-6 space-y-4 text-sm">
                <div className="flex justify-between gap-4"><dt className="text-white/50">Startbudget</dt><dd className="font-semibold">{formatMoneyFromSek(startingBudgetSek, currency)}</dd></div>
                ...
                <div className="flex justify-between gap-4"><dt className="text-white/50">Totalt</dt><dd className="font-semibold">{formatMoneyFromSek(total, currency)}</dd></div>
                <div className="flex justify-between gap-4"><dt className="text-white/50">Kvar</dt><dd className="font-semibold text-[var(--gold)]">{formatMoneyFromSek(remaining, currency)}</dd></div>
              </dl>
