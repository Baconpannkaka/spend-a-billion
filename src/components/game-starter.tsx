...
export function GameStarter({ mode }: { mode: GameMode }) {
  ...
  async function launch(setup: GameSetup | null) {
    if (!setup) { setError("Kontrollera beloppet och försök igen."); return; }
    setError("");
    setLoading(true);
      ...
      <div className="shell">
        <div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr]">
          <div className="lg:sticky lg:top-24 lg:self-start">
            <p className="eyebrow text-[var(--gold-dark)]">{mode === "luxury" ? "Miljardärsläge" : "Vardagsläge"}</p>
            <h1 className="mt-3 font-display text-5xl leading-[.95] md:text-6xl">{mode === "luxury" ? "Hur rik vill du vara?" : "Hur stor är shoppingkassan?"}</h1>
            <p className="mt-4 max-w-lg text-sm leading-7 text-black/55">{mode === "luxury" ? "Välj en uppskattad förmögenhet, ta miljarden direkt eller räkna på din aktiedröm." : "Välj en färdig vardagsbudget, ange ett eget belopp eller låt en aktiedröm finansiera kundvagnen."}</p>
            <div className="mt-6 flex flex-wrap gap-2">
              {mode === "luxury" && <button type="button" onClick={() => setTab("profiles")} className={`tab-button ${tab === "profiles" ? "is-active" : ""}`}><Crown className="h-4 w-4" /> Be as rich as</button>}
              <button type="button" onClick={() => setTab("custom")} className={`tab-button ${tab === "custom" ? "is-active" : ""}`}><WalletCards className="h-4 w-4" /> {mode === "luxury" ? "Egen budget" : "Välj budget"}</button>
              <button type="button" onClick={() => setTab("stock")} className={`tab-button ${tab === "stock" ? "is-active" : ""}`}><BarChart3 className="h-4 w-4" /> Get rich on stock</button>
            </div>
            <p className="mt-5 text-xs leading-5 text-black/40">Förmögenheter, priser och växelkurser är avrundade underhållningsvärden. Inga finansiella råd eller riktiga köp.</p>
          </div>

          <div>
            {tab === "profiles" && mode === "luxury" && <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <button type="button" onClick={() => void launch(createClassicSetup(currency))} className="profile-card bg-[var(--ink)] text-white"><div className="avatar">1B</div><p className="mt-4 text-xs uppercase tracking-[.14em] text-[var(--gold)]">Klassikern</p><h2 className="mt-1 font-display text-2xl">En miljard</h2><p className="mt-2 text-sm text-white/50">{formatMoneyFromSek(1_000_000_000, currency)}</p></button>
              {wealthProfiles.map((profile) => {
                const budgetSek = currencyToSek(profile.netWorthUsd, "USD");
                return <button key={profile.id} type="button" onClick={() => void launch(createPersonSetup(profile.id, currency))} className="profile-card"><div className="avatar bg-black/5 text-black">{profile.initials}</div><p className="mt-4 text-xs uppercase tracking-[.12em] text-black/40">{profile.profession}</p><h2 className="mt-1 font-display text-2xl">{profile.name}</h2><p className="mt-2 text-sm font-semibold text-[var(--gold-dark)]">{formatCompactMoneyFromSek(budgetSek, currency)}</p></button>;
              })}
            </div>}
            ...
            {tab === "custom" && <div className="rounded-xl border border-black/10 bg-white p-5 md:p-7">
              {mode === "everyday" && <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{everydayPresets.map((preset) => {
                ...
                return <button key={preset.id} type="button" onClick={() => void launch(setup)} className="budget-card"><p className="text-xs font-bold uppercase tracking-[.13em] text-[var(--gold-dark)]">{preset.label}</p><p className="mt-2 font-display text-3xl">{formatMoneyFromSek(setup.startingBudgetSek, currency)}</p><p className="mt-2 text-sm leading-5 text-black/50">{preset.copy}</p></button>;
              })}</div>}
              <div className={mode === "everyday" ? "mt-6 border-t border-black/10 pt-6" : ""}><p className="text-xs font-bold uppercase tracking-[.13em] text-[var(--gold-dark)]">Eget belopp i {currency}</p><div className="mt-3 flex flex-col gap-3 sm:flex-row"><input inputMode="decimal" value={customAmount} onChange={(event) => setCustomAmount(event.target.value)} className="field flex-1" aria-label="Eget budgetbelopp" /><button type="button" disabled={loading} onClick={() => void launch(createCustomSetup(mode, Number(customAmount), currency))} className="primary-button"><Coins className="h-4 w-4" /> Starta med beloppet</button></div></div>
            </div>}

            {tab === "stock" && <div className="rounded-xl border border-black/10 bg-white p-5 md:p-7"><div className="grid gap-4 sm:grid-cols-2"><label className="field-label">Aktie eller bolag<input value={stockName} onChange={(event) => setStockName(event.target.value)} className="field mt-1.5" /></label><label className="field-label">Antal aktier<input inputMode="numeric" value={shares} onChange={(event) => setShares(event.target.value)} className="field mt-1.5" /></label><label className="field-label">Drömkurs per aktie<input inputMode="decimal" value={targetPrice} onChange={(event) => setTargetPrice(event.target.value)} className="field mt-1.5" /></label><label className="field-label">Aktiens valuta<select value={quoteCurrency} onChange={(event) => setQuoteCurrency(event.target.value as CurrencyCode)} className="field mt-1.5">{currencies.map((entry) => <option key={entry.code} value={entry.code}>{entry.code} – {entry.name}</option>)}</select></label></div><div className="mt-5 flex flex-col justify-between gap-4 rounded-lg bg-[var(--paper)] p-4 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[.13em] text-black/40">Du får att handla för</p><p className="mt-1 font-display text-3xl">{stockPreview > 0 ? formatMoneyFromSek(stockPreview, currency) : "–"}</p></div><button type="button" disabled={loading} onClick={() => void launch(createStockSetup({ mode, stockName, shares: Number(shares), targetPrice: Number(targetPrice), quoteCurrency, currency }))} className="primary-button">Starta aktiedrömmen <ArrowRight className="h-4 w-4" /></button></div></div>}
            {error && <p role="alert" className="mt-3 text-sm font-semibold text-red-700">{error}</p>}
            {loading && <div className="mt-4 flex items-center gap-2 text-sm text-black/50"><LoaderCircle className="h-4 w-4 animate-spin" /> Laddar 10 000 produkter…</div>}
          </div>
        </div>
      </div>
    </section>
