...
export function sanitizeCart(cart: CartItem[], products: Product[]): CartItem[] {
  ...
  for (const item of cart) {
    if (!productIds.has(item.productId) || !Number.isInteger(item.quantity) || item.quantity <= 0) {
      continue;
    }
...
export function getRemainingBudget(cart: CartItem[], products: Product[]): number {
  return Math.max(0, STARTING_BUDGET_SEK - getCartTotal(cart, products));
}

export function getSpentRatio(cart: CartItem[], products: Product[]): number {
  return getCartTotal(cart, products) / STARTING_BUDGET_SEK;
}
...
export function canAddQuantity(
  ...
  return getCartTotal(cart, products) + product.priceSek * quantity <= STARTING_BUDGET_SEK;
}
