...
describe("kundvagn", () => {
  it("räknar totalsumma och återstående budget", () => {
    ...
    expect(getRemainingBudget(cart, products)).toBe(998_200_000);
  });

  it("hindrar köp som överskrider budgeten", () => {
    const cart = [{ productId: gulfstream.id, quantity: 1 }];
    expect(canAddQuantity(cart, products, gulfstream.id, 1)).toBe(false);
    expect(canAddQuantity(cart, products, rolex.id, 1)).toBe(true);
  });
