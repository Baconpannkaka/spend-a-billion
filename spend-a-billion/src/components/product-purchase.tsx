...
export function ProductPurchase({ product }: { product: Product }) {
  const { remaining, addItem } = useGame();
  ...
  const purchase = () => {
    ...
    if (result.ok) {
      ...
      showToast(`Det där spräckte budgeten. Du saknar ${formatSek(result.missing)}.`, "error");
    }
  };
    ...
    <div className="rounded-2xl border border-black/10 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 flex items-end justify-between gap-4 border-t border-black/10 pt-5">
        <div>
          ...
          <p className="font-display text-3xl">{formatSek(product.priceSek * quantity)}</p>
        </div>
