...
export function CheckoutClient() {
  const { cart, total, remaining, name, setName, completePurchase, hydrated } = useGame();
      ...
      <div className="shell">
        ...
        <div className="mt-7 grid gap-8 lg:grid-cols-[1.05fr_.95fr] lg:items-start">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-lg leading-8 text-black/60">Välj namnet som ska stå på ditt resultat. Kortuppgifterna är visuella, låsta och fullständigt påhittade.</p>

            <label className="mt-8 block max-w-xl">
              <span className="mb-2 block text-sm font-semibold">Namn på miljardären</span>
          ...
          <aside className="sticky top-24 space-y-5">
            <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-xl">
              ...
              <dl className="mt-6 space-y-4">
                <div className="flex justify-between gap-4"><dt className="text-black/50">Fantasiköp</dt><dd className="font-semibold">{formatSek(total)}</dd></div>
                <div className="flex justify-between gap-4"><dt className="text-black/50">Kvar efter köp</dt><dd className="font-semibold text-[var(--gold-dark)]">{formatSek(remaining)}</dd></div>
              </dl>
